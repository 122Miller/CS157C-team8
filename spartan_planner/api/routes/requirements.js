...
router.get('/', async (req, res) => {
    const courses = await Requirement.find().select('-_id');
    res.send(courses);
})
...
router.post('/', async (req, res) => {
    ...
    const exist = await Requirement.findOne({ course: req.body.course });
    if(exist) return res.status(400).send('Course already registered');

    const course = new Requirement({
...
router.delete('/:course', async(req, res) => {
    const findCourse = await Requirement.findOneAndDelete({course: req.params.course})
    if(!findCourse) return res.status(404).send('Course was not found')
    res.send(findCourse)
})

router.put('/:course', async(req, res) => {
    const findCourse = await Requirement.findOneAndUpdate({ course: req.params.course }, req.body, {new: true})
    if(!findCourse) return res.status(404).send('Course was not found')
    res.send(findCourse)
})
